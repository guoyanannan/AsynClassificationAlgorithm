...
def thread_process_model_res(db_ip, db_user, db_psd,db_name,thread_obj,
    ...
    while True:
        ...
        if not index_q.empty():
            ...
            v2 = time.time()
            batch_result_iter = model.inference_asyn(batch_img_path, image_arr, image_list,use_clsfier)
            v3 = time.time()
            ...
            if debug:
                ...
                if schema_flag ==0:
                    ...
                    db_oper_.write_cam_defects(db_defects_info)
                v6 = time.time()
                get_q_data += v2 - v1
                get_model_res += v3 - v2
                get_pro_res += v4 - v3
...
def parse_opt():
    ...
    parser.add_argument('--save_intercls', type=str, default=cls_config['filePath']['internal_number'], help='internal number to save')
    ...
    parser.add_argument('--use_classifier', action='store_true', default=cls_config['infoParameter']['use_classifier'], help='is or not use classifier')
